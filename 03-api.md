...
| | |
|---|---|
| `GET /settings` | model routing for **all 3 groups** (`chat`/`conspect`/`quick`, ADR-025 + [ADR-043](adr/043-quick-routing-tier-m4.md)) + available models/providers per group incl. which support effort + their levels (registry-sourced; no hardcoded lists in web) |
| `PUT /settings/models` | save **one** group's `{active, fallback, effort_by_provider}`; `422` on unknown id; busts the routing cache (forward-live, no restart) |
...
| `GET /admin/providers` | **(M4 follow-up, [ADR-044](adr/044-provider-observability-surface.md))** provider observability — one row per registered provider: `{ id, label, capabilities:[chat\|stt\|embedding], reachable, last_error:{message,at}\|null, last_success_at, consecutive_failures }`. `reachable` is a **live `Provider.health()` probe** (config-reachability, **not** a success guarantee); `last_error` is **sticky** (a later success does not clear it) with `last_success_at`/`consecutive_failures` beside it — state is **in-memory** on the registry (resets on redeploy, no persistence). Session-gated; **no LLM call**. `/health` stays public + untouched |
