...
| | |
|---|---|
| `GET /media/{id}` | **(M9, ADR-057 §7)** streams a stored media file (photo/voice/thumbnail); session-gated |
...
| `GET /entities?type=person&q=…` | **(M9.5, ADR-058 §11)** alphabetical browse/search over entity hubs — feeds the merge-UI pickers (search-as-you-type; `/search` stays query-shaped) |
...
> (like standing manual merges, [ADR-042](adr/042-reprocess-all-from-raw-and-data-survival.md); the
> **anchor** edit is reprocess-safe — it writes `captures.created_at`, which the replay reads).
