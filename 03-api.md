...
> **M9.8 addendum (T5.5 — orphan keep-list + offender `type`, [ADR-064](adr/064-durable-merges-visual-dedup-gc.md) §5):**
> the graph-health **`orphan-nodes`** check's offender samples gain a **`type`** field
> (`{ id, label, type }`) so the web card can tell **hubs** (get inline Keep/Merge) from **content**
> nodes (Delete→capture-remove only). The check's count+sample now **exclude any kept hub** — a hub
> whose normalized `surface_forms` intersect an `orphan_keeps` decision of the same type (the
> read-time whitelist filter); kept hubs live only in `GET /admin/orphan-keeps`, not the finding. The
> other five node-checks are unaffected. Keep/un-keep endpoints are in §Admin above.
...
| `POST /admin/nodes/{id}/keep` | **(M9.8 T5.5, [ADR-064](adr/064-durable-merges-visual-dedup-gc.md) §5)** **keep** (whitelist) a zero-degree orphan **hub** so the nightly graph-health orphan check stops flagging it (e.g. Father/Mother). **Synchronous** (no `agent_runs` job — a config-like decision): the server resolves the node's current `surface_forms` + type and **upserts** an `orphan_keeps` decision keyed on **surface form + type, not node id** (survives `reprocess-all`; §1's posture). `200`/`204`; `404` unknown/tombstone; `400` a **content** node (Keep is **hubs-only**). Idempotent |
| `GET /admin/orphan-keeps` | **(M9.8 T5.5)** the kept-hub list `[{key, type, label, kept_at}]` backing the web "Kept (N)" strip. `key` is the stable `keep_key` (surface-form+type identity) used to un-keep — a **base64url**-encoded opaque handle (URL-path-safe: it rides in the `DELETE` path, where the raw NUL-joined canonical form or a `/` in a surface form would be rejected/normalized by the ingress); `label` is the hub's title form |
| `DELETE /admin/orphan-keeps/{key}` | **(M9.8 T5.5)** **un-keep** — remove the `orphan_keeps` decision by its stable `keep_key` (**not** node id: a reprocess changes the id, the key persists). `204`; `404` no such key. The hub reappears in the orphan check on the next run |
