...
| `GET /admin/orphan-keeps` | **(M9.8 T5.5)** the kept-hub list `[{key, type, label, kept_at}]` backing the web "Kept (N)" strip. `key` is the stable `keep_key` (surface-form+type identity) used to un-keep |
